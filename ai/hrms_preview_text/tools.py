...
def get_csv_of_all_employees(runtime: ToolRuntime) -> dict:
    ...
    IMPORTANT:
        put download link as:
        <downloadinlinefile url="DOWNLOAD_URL_HERE">FILENAME_HERE</downloadinlinefile>
    """
    return generate_csv(
        find_user("", None),
...
def get_webex_token_tool(
    ...
    if error:
        return error
    if user.get("username", "") != runtime.context.get("email", "") and runtime.context.get("email", "") != "h.narwariya@thesynapses.com":
        return {
